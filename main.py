...
API_TOKEN = '7057386871:AAFsXhnsLzJ5TfzCNx-DC5aCaGAuq8yIZZI'

bot = tb.TeleBot(API_TOKEN)

# Зберігання анкет користувачів
users = {}
...
# Обробка натискань кнопок "Лайк" і "Дизлайк"
@bot.callback_query_handler(func=lambda call: call.data.startswith("like_") or call.data.startswith("dislike_"))
def handle_vote(call):
    action, user_id = call.data.split("_")
    liker_id = call.message.chat.id  # ID користувача, який поставив лайк або дизлайк
    if action == "like":
        bot.answer_callback_query(call.id, "Ви поставили лайк!")

        # Надсилаємо повідомлення користувачу, якому поставили лайк
